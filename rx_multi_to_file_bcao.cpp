// This is for Bing Cao, not to be complied or exectued.
...
#include "mu_meta.hpp"
 ...
 // Size of each data buffer in bytes (256 MB)
#define BUFFER_BYTES 268435456

// Namespace aliasing to simplify usage
...
// Struct for holding all XML configuration parameters for one USRP multi-stream
// Includes settings for frequency, gain, antenna, sample rate, metadata, file paths, etc.
...
// Class for parsing and holding multiple USRP configurations from XML
// Method to load and parse XML into a list of usrp_multi_args
      ...
      if(not std::strncmp(usrp_multi.first.data(), "usrp_multi", 10)) {
        ...
        for(auto stream : stream_nodes) {
          if(not std::strncmp(stream.first.data(), "stream", 6)) {
            ...
            args.offset_freq.push_back(stream.second.get<double>("offset_freq", 0.0));
...
// Class that wraps USRP device, sets configuration, handles streaming, buffering, and file output
// Constructors support various copy and move semantics
// Main `thread_main<type>()` performs the streaming work in a thread-safe manner
// Includes functions for checking clock lock, triggering, naming output files, and stopping

// Shared memory structure used for inter-process triggering
...
public:
  uhd_cbuffer_multi(usrp_multi_args& args, bool has_pps = false) :
    ...
    _offset_freq_hz(args.offset_freq),
  ...
  uhd_cbuffer_multi(uhd_cbuffer_multi& other) :
    ...
    _offset_freq_hz(other._offset_freq_hz),
  ...
  template<typename type> void thread_main(void) {
...
// Main entry point using UHD_SAFE_MAIN for proper signal handling
  // Define and parse command line options
  // Load XML settings file containing USRP config
  // Instantiate uhd_cbuffer_multi objects, each representing one USRP config block
  // Configure USRP clock/time sources, sample rates, gain, antenna, etc.
  // Wait for USRPs to become ready and check LO/REF lock
  // Launch threads, one per USRP config, to record data
  // Optionally wait for start time or shared memory trigger
  // When trigger is active, start recording to disk
  // Handle SIGINT signal for graceful shutdown
  // Stop all threads and close all streams on exit
